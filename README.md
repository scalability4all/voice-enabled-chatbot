...
* Configure Weather API Key
    - Go to the [Open WeatherMap api website](https://openweathermap.org/).
    - Sign Up and Create account.
    - Click on API keys option from the menu button
    - Copy the Key that is generated by default.
    - Paste the API key in [`config.py`](/config.py)file.
